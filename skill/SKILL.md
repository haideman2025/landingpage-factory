...
## CẬP NHẬT v18 (editor sửa toàn bộ section + combo linh hoạt) ✅
1. **Combo = dữ liệu sửa được** (`CFG.bundles[]` thay vì tính cứng): mỗi combo {q,label,price,orig,gift,badge,def}; sinh mặc định 3 combo từ đơn giá trong genConfigs; renderLP render combo cards từ `CFG.bundles` (defB = combo có def → checked + hidden qty/bval + tổng mặc định).
2. **Editor mở rộng (`edExtras`)**: khối "Sửa nội dung mọi section + combo" với các nhóm `<details>` gập: Nỗi đau, Đánh giá (thêm/xoá/sửa tên+tuổi+nội dung), Lý do, FAQ, USP, Tầng hương (thêm/xoá), Section chi tiết SP, **Combo khuyến mãi** (sửa tên/SL/giá/giá gốc/quà/badge + chọn mặc định + thêm/xoá), Nhãn chữ chung. Helper `_in/_ta/_num/_g`; per-LP bind vào `LPS[i].a.*`, dùng-chung bind vào `UI.*`/`CFG.*`; nút thêm/xoá gọi `openEditor(i)` để refresh. Sửa xong bấm Preview/Tải là áp dụng ngay (renderLP đọc CFG/UI/LPS live).
- Verify: node --check + test (combo render từ CFG.bundles + default checked, editor có đủ field combo/scents/usp/details/pains/testi/reasons/faqs/UI + add/remove wired) + regression (xanh).

## CẬP NHẬT v19 (ảnh per-USP + ảnh quà + benefit có mô tả + before/after có text) ✅
1. **Mỗi section chi tiết SP có ảnh riêng**: details schema thêm `scene`; buildSlots tạo slot `detail0..N` (scene từ d.scene) khi có details; render mỗi detail = card ảnh-dẫn (ảnh 4/3 + tiêu đề + mô tả) → bớt dày chữ, kể chuyện bằng hình.
2. **Upload ảnh quà tặng cho combo**: `CFG.bundles[j].giftImg/giftImgUrl`; editor combo có nút upload ảnh quà (`uploadGift()` crop 1:1); combo card hiện ảnh quà (flex: ảnh + nội dung); lưu `assets/gift-N.jpg` vào DEPLOY KIT.
3. **Benefit "Vì sao chọn" có tiêu đề + mô tả**: `ui.reasons` đổi từ mảng string → `[{t,d}]` (sửa lỗi AI echo "r1..r4"); render `.ben` = icon + tiêu đề + mô tả; editor sửa t/d + thêm/xoá; normalize tương thích string cũ.
4. **Before/After có text lợi ích**: thêm `ui.before_d` (pain trước) + `ui.after_d` (lợi ích sau) → overlay vào 2 scene full-bleed để so sánh cảm xúc nhanh; sửa được trong editor.
- Verify: node --check + render/editor tests (detail img, gift img asset+preview, benefit t/d, before/after text, legacy string reasons) + regression (xanh).

## CẬP NHẬT v20 (fix Preview không play được video TikTok) ✅
Root cause (debug): `preview()` render LP bằng **iframe `srcdoc`** → document có **origin opaque (null)** → iframe TikTok lồng bên trong (lightbox) không thiết lập được origin/referrer nên không play (trang deploy thật vẫn play vì có origin https). Fix: đổi Preview sang **Blob URL** (`URL.createObjectURL(new Blob([html],{type:'text/html'}))` → iframe kế thừa origin https của tool) + cấp `allow="autoplay; fullscreen; encrypted-media; picture-in-picture"` cho iframe preview và thêm `fullscreen` vào allow của iframe lightbox; revoke blob cũ mỗi lần preview; fallback srcdoc nếu lỗi. Verify: node --check + test preview tạo Blob URL text/html + tiktok card → openVid embed.

## CẬP NHẬT v21 (biểu tượng hero → band số liệu mở rộng + số chạy + sửa được) ✅
3 chip hero cũ (🏆 Top 1 / 👥 400K+ / ⭐ 5.0, tĩnh) → bỏ chip, thêm **band `.hstats`** ngay sau hero: mỗi `CFG.heroStats[]` = {icon, count, suf, dec, val, label, desc} render thành thẻ icon + **số chạy đếm lên** (reuse IntersectionObserver `[data-count]`; `cup()` nâng cấp hỗ trợ `data-dec` thập phân + suffix → 400→"400K+", 5→"5.0", "TOP 1" giữ chữ) + nhãn + **mô tả mở rộng**, reveal khi cuộn. Default bake nội dung ONIIZ anh đưa (Top1 sàn TMĐT TikTok/Shopee, 400K+ khách tin tưởng, 5.0 đánh giá TB mọi nền tảng). Editor: nhóm "🏅 Biểu tượng/số liệu hero" sửa icon/số chạy/hậu tố/số lẻ/chữ/nhãn/mô tả + thêm/xoá. Verify: node --check + render (số chạy 400K+/5.0/TOP 1, mô tả, chip cũ đã bỏ) + editor + regression.
